...
2. 새 SC/GA 데이터로 이번 세션 보강분(dryer 등) 반영 여부 확인
3. `.table-scroll` 미닫힘 버그가 반복적으로 나오고 있음 — 표 만드는 템플릿/습관 자체를 점검해볼 것 (매번 사후 발견하기보다 애초에 안 만들도록)
4. 사용자가 결과 화면 스크린샷 주면 숫자 재검산하는 습관 유지 (v10에서 electric-bill-spike-calculator 계산 로직 버그 이렇게 발견함)
