...
- blog: 24개 (v14에서 are-carbon-offsets-worth-it.html 신규 추가)
- compare: 41개
...
8. **버그 재발 패턴 정리**: article-body 조기종료 버그가 v10/v11/v12에서 계속 나왔음(v13/v14는 재스캔 결과 0건이지만 v14에서 `<h2>` 짝 안 맞는 유사 패턴이 새로 나왔음 — solar-incentives-by-state.html). 새 세션 시작하면 항상 정규식 재스캔부터 할 것: `re.findall(r'</p>\s*\n\s*</div>\s*\n\s*\n\s{10,}<h[23]>', content)`. 정적 3열 이상 비교표에 `table-scroll` 랩퍼 누락되는 패턴도 발견됨(hot-tub-running-cost.html) — 새 비교표 만들 때마다 `<div class="table-scroll"><table>...</table></div>`로 감싸는 걸 습관화할 것. **JS 계산기에서 `parseFloat(x)||기본값` 패턴도 위험 — 사용자가 유효하게 0을 입력해도 기본값으로 대체되는 버그 가능(ev-vs-gas-car.html에서 발견) — `isNaN()` 체크로 대체할 것.**
