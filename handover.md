...
**GSC 밖 탐색.** 2026 겨울 대비 보도(Generac 발표, NOAA 겨울 전망, 유틸리티 경보)에서 "난방이 멈추면 얼마나 버티나"가 반복 등장. 커버리지 `heat loss rate` / `cooldown` / `power goes out` **전부 0건**.

### ⭐ 경쟁 회피 장치 — 같은 렌즈가 4번째로 통했다
경쟁이 **전부 HVAC·배관 업체 리드젠**(One Hour Heating, Aire Serv, Arlinghaus, Ostrom, Naughton, Access Heating). v23(공급사 제휴) → v26(부동산관리 SW) → v29(HVAC 리드젠) → v31. **"업계용만 있고 소비자용 계산기가 없는 영역" 렌즈는 이제 검증된 발굴 공식이다.**

결정적 발견: **이들의 답이 서로 다르다.** "6~10시간"(One Hour), "8~12"(Aire Serv), "12~24"(WRAL), "보통 24시간 내"(Naughton). 그리고 **아무도 단열 수준과 외기온도를 묻지 않는다** — 결과를 지배하는 두 변수를 전부 생략.

### 차별화 3가지 (유지할 포지션)
1. **뉴턴 냉각법칙 기반 실제 계산.** 시정수(τ)를 건축 연대별 5단계(10/15/22/32/45h)로 두고 풍속 보정(×1.0/0.85/0.72).
   - **검증이 핵심**: 평균집·외기 20°F에서 55°F까지 **6.9시간** → 업체들의 "6~10시간"을 정확히 재현. 즉 모델이 그들 주장과 일치하면서, **동시에 실제 범위가 2.1h(낡은집·0°F)~28.1h(고성능·40°F)임을 드러낸다.** "6~24시간"이 가리고 있던 것.
2. **예열 효과 정량화** — 68→75°F로 미리 올리면 평균집 기준 **+3.0시간** 확보(검산 일치). 무료이고 실행 가능한데 아무도 수치로 안 줌.
3. **임계온도 4단계**(60°F 결로 / 55°F 배관 / 50°F 저체온 / 40°F 거주불가)를 **v28 개인난방 계산기와 동일 기준으로 통일.** 클러스터 간 일관성 유지 — 앞으로 온도 임계값을 쓸 땐 이 4단계를 재사용할 것.

### ★ 안전장치 (절대 톤 낮추지 말 것)
이 주제는 사람이 죽는 영역이다. 다음을 유지할 것:
- **발전기 실내·차고 금지, 백피드 금지, 오븐·그릴 난방 금지, CO 경보기 권고**를 툴·블로그 양쪽에 명시
- **"배관은 온도조절기보다 차갑다"** — 실제 터지는 배관은 외벽·크롤스페이스·다락이라 실내 타임라인보다 먼저(4~6h) 언다는 점을 **결과문에도** 넣어 오독 방지
- **"40°F 이하면 나가라, 대피소 가는 건 실패가 아니다"** 명시
- 계산기 한계(단일 공간 가정, τ는 연대별 추정치이지 실측 아님) 결과문에 기재

### 구성
`tools/power-outage-house-cooling-calculator.html` / `blog/how-long-will-my-house-stay-warm-without-power.html`
compare는 만들지 않음 — 기존 `compare/solar-vs-generator.html`이 백업전원 비교를 이미 담당하므로 카니발 방지. 대신 양방향 링크로 연결.

### 부수 효과
단열의 **"정전 회복력"** 가치를 명시했다 — 페이백 계산에 안 잡히는 편익이라 `insulation-savings`에 새 각도를 준다. 향후 단열 콘텐츠에서 재사용 가능.

### 계절성
v28 개인난방, v29 AUX와 함께 **난방 시즌 3종 세트**가 됐다. 이벤트성(한파·정전 발생 시 급증)이라 **11~2월 데이터로 함께 측정할 것.** 지금 심는 게 타이밍상 맞다.
...
**GSC 밖 탐색.** 2026 겨울 요금 보도에서 *"히트펌프 소유자가 한파에 보조 전기열이 켜지면서 특히 크게 당했다"*는 구체적 신호를 포착.

### 중복 확인
`auxiliary heat` / `aux heat` / `emergency heat` / `heat strip` / `balance point` **전부 0건**. **히트펌프 페이지가 이미 4개나 있었는데 정작 요금 폭탄의 실제 원인이 누락돼 있었음** — 기존 클러스터에 구멍이 나 있는 패턴이니, 다음에도 "우리가 이미 다루는 주제인데 핵심 메커니즘이 빠져 있지 않은가"를 점검할 것.

### 경쟁 회피 장치 (유지할 포지션)
경쟁은 **전부 HVAC 업체·전기조합 리드젠 페이지**(Craighead Electric, Jackson EMC, Watkins, tbyrd, Charlotte Comfort 등 — 하나같이 "전화주세요"로 끝남). 전부 "AUX는 2~5배 비싸다"고만 하고 **계산기가 없음.**
→ **v23(공급사 제휴만 존재), v26(부동산관리 SW만 존재)과 동일한 "업계용만 있고 소비자용 없음" 비대칭. 이 렌즈가 세 번째로 통했다.**

1. **실제 금액 산출** — 스마트 온도조절기(Ecobee/Nest)가 보고하는 aux 런타임 시간을 입력받음. 60h/10kW → 추가 **$108**. 정상 4kW $0.72/h vs aux 14kW $2.52/h = **3.5배**.
2. **진단 기능** — aux가 난방시간의 몇 %인지로 3단계 판정(18% 정상 / 57% 이상징후 / EM heat 사용 시 별도 경고). "설정 문제인가 고장인가"를 구분. 원인을 빈도순 제시(온도조절기 lockout 미설정 > 과도한 setback > 냉매 부족 > 제상 고장 > 리버싱밸브 고착).
3. **AUX vs EM heat 반직관 정량화** — EM이 전력은 **덜** 쓰는데($1.80/h vs $2.52/h) 열 단위당은 **더** 비쌈($0.18 vs $0.12). 히트펌프를 꺼버리기 때문. 경쟁사는 "더 비싸다"고만 하고 이유를 수치로 안 보여줌.

### ★★ 자체 조언 정정 (무결성 — 중요)
**v28에서 만든 `personal-heating-savings-calculator`가 DOE 기준 "7~10°F setback"을 권하는데, 이는 furnace 전제라 히트펌프+저항열 사용자에게는 역효과다.** 깊은 setback → 회복 시 strips 작동 → setback으로 아낀 것보다 더 지출. 계산기 note와 `best-thermostat-settings-to-save-money` 본문에 **"히트펌프는 2~3°F"** 경고를 추가했다.
→ **교훈: 신규 클러스터가 기존 조언의 결함을 드러낼 수 있다. 새 주제를 팔 때 기존 페이지가 그 주제와 모순되지 않는지 반드시 역방향으로 점검할 것.** (v27의 driving-vs-flying 모델 결함 발견과 같은 계열)

### 구성
`tools/heat-pump-aux-heat-calculator.html` / `blog/why-is-my-heat-pump-electric-bill-so-high.html` / `compare/auxiliary-heat-vs-emergency-heat.html`

### 계절성
난방 시즌(11~2월) 한파 때 검색이 몰리는 **이벤트성 쿼리**다. v28 개인난방과 함께 11~12월 데이터로 측정할 것. 지금 심어두는 게 타이밍상 맞다.

### 균형 서술 (유지할 것)
"제대로 설정된 히트펌프는 여전히 저항열보다 훨씬 싸고 대부분 지역에서 가스와 경쟁력 있다 — 문제는 strips가 필요 이상으로 도는 것이지 히트펌프 개념의 실패가 아니다"를 블로그에 명시했다. 요금 충격을 겪은 사용자가 히트펌프 자체를 후회하게 만들지 않도록 한 장치이니 유지할 것.

---

# EcoEnergyCalc 인수인계 — v28 세션 (2026-09-28)

## 0-T. ✅ v28 신규 클러스터: 개인난방(Personal Heating) — 커밋 `34de6ed`, 140 → 143파일

**GSC 밖 탐색.** 후보 여럿(셉틱·우물 설치비, 수영장 물, 홈오피스, 썸프펌프)을 경쟁·코어 적합성으로 걸러 선정. `electric blanket`/`heated blanket`/`portable heater`가 **사이트 전체 0건**이면서 난방비 상승기와 맞물린 "heat the person, not the house" 각도.

### 중복 확인
- 개인난방 기기류 0건.
- `blog/best-thermostat-settings-to-save-money.html`이 setback을 산문으로 다루지만 **계산기 없음**(`calc-box` 0). 카니발 없이 보완재로 성립하며, 이 블로그 본문에서 신규 툴로 역링크 추가함.

### ⭐ 경쟁 회피 장치 (유지할 포지션)
경쟁은 heavy하지만 **거의 전부 영국**(Uswitch, MoneySuperMarket, EDF, VonHaus, NimbleFins, ClearScore) — 펜스·Ofgem 요금상한 기준이라 미국 사용자에게 사실상 무용. 미국 콘텐츠는 얇고 요금도 낡음($0.12/kWh 등).
1. **미국 단위·2026 요금**(18.4¢/kWh, therm, °F) — 영국 지배 니치에 대한 근본 차별. **"경쟁이 많아 보여도 시장(국가)이 다르면 비어 있다"는 새 발굴 렌즈. v26의 '업계용만 있고 소비자용 없음' 렌즈와 함께 재사용할 것.**
2. **순 계산: setback 절감 − 기기 전기료.** 경쟁은 전부 "기기가 시간당 2센트"까지만 계산하고 멈춤. 실제 결정은 "온도를 몇 도 내릴 수 있나"인데 아무도 답을 안 함. DOE 1%/°F 기준으로 순액 산출(담요 $22 지출 vs setback $84 절감 = 순 $62). **"담요 사고 온도 안 내리면 $22 손해"라는 실패 모드를 전면 배치.**
3. **안전 하한 3단계** — 55°F 동파 / 60°F 결로·곰팡이 / 64°F WHO 최소(영유아·고령자 68°F). 영국 경쟁 글들은 곰팡이를 언급만 하고 임계값을 안 줌. 입력 설정온도에 따라 계산기가 자동 경고(52/58/62/66°F 4단계 전부 발화 검증 완료).

### 정직성 장치 (건드리지 말 것)
- **개인난방은 "더 추운 집을 감수해서 아끼는 것"**이고 단열은 그 트레이드오프 없이 아낀다는 점을 명시, `insulation-savings`로 연결.
- **공간히터는 가스·히트펌프 중앙난방 대비 BTU당 약 3배 비싼 연료**라는 점을 compare에 명시 — 경쟁이 "100% 효율"로 뭉개는 지점. 단 전기저항 중앙난방이면 정직하게 역전됨을 계산기가 보여줌.

### 검증
전 계산기 node 실행 + 수동 교차검산 일치. **두 계산기가 독립 경로로 담요 순 $62를 동일 산출.** 공간히터는 표준조건 순 −$204이나 전기저항난방+큰 setback($3,000/18°F)에서 +$28로 역전.

### 구성
`tools/personal-heating-savings-calculator.html` / `blog/is-an-electric-blanket-cheaper-than-heating.html` / `compare/electric-blanket-vs-space-heater.html`

### 계절성 참고
이 클러스터는 **북반구 난방 시즌(10~3월)에 트래픽이 집중**된다. 지금 심는 게 타이밍상 맞고, 효과 측정은 11~12월 데이터로 할 것. 여름에 성과가 낮다고 실패로 판단하지 말 것.
...
**GSC 밖 탐색.** 레딧·지역뉴스의 세입자 사례에서 발굴: 미시간 세입자가 공용 지하실(집주인이 히터 틀고 TV 보던 공간)이 **본인 회로**에 물려있던 것을 발견한 건, 뉴헤이븐 $702 전기요금 시위, 텍사스 세입자 요금 4배 급등. 공통점은 **"내 청구액이 맞는지 검증할 방법이 없다"**.

### ⭐ 이번 발견의 핵심: 콘텐츠 생태계의 비대칭
검색 상위가 **doorloop / azibo / nchm / hemlane / rentlane / hoozzee — 전부 부동산관리 SW 벤더**이고, 내용이 하나같이 *"임대인이 유틸리티 비용을 회수하는 법"*. **세입자 측 검증 도구가 전무함.** 이건 우리가 이미 두 번 써먹은 패턴과 동일: supply-vs-delivery(제휴 사이트 대응), balcony solar(벤더 키트 대응). **"업계용 콘텐츠만 있고 소비자용이 없는 영역"을 찾는 게 이 사이트의 재현 가능한 발굴 공식이 됐다. 다음 세션도 이 렌즈를 우선 적용할 것.**

### 차별화 3가지 (유지할 포지션)
1. **5개 배분식 동시 산출**(균등/면적/인원/침실/하이브리드) — 어느 식이냐에 따라 같은 집이 $136~158로 갈리는 걸 보여줌. 배분식은 법으로 정해진 바가 없어서 "불공평하다"는 주장 자체가 법적 근거가 못 됨.
2. **초과회수 탐지 (이게 실제 무기)** — 청구액÷배분지분으로 건물 전체 역산. 대부분 주에서 **"전 세입자 청구 합계가 실제 청구서를 초과할 수 없다"**가 유일하게 강제력 있는 조항이라, 공정성 논쟁 대신 **산술 불일치**를 잡아줌. 검증: 면적 5% 지분에 $205 청구 → 역산 $4,100 vs 실제 $3,000 = **+30% 초과** 탐지.
3. **RUBS와 공용미터(shared meter) 구분** — 후자는 다수 주에서 위법이라 훨씬 강한 카드인데 대중적으로 혼동됨. 위 미시간 사례가 정확히 후자.

### 안전장치 (건드리지 말 것)
- **"이 도구는 산술을 검증하지 법을 판단하지 않는다"** 명시. 배분식은 법정 사항이 아니고 주별로 다르므로 주 법무장관/세입자 단체로 안내.
- **정직성**: RUBS가 무조건 나쁜 게 아니라 *효율적 가구엔 불리, 과다사용 가구엔 유리*하다는 비대칭을 compare에 명시. 한쪽 편들지 않음.
- **면적·침실 방식에선 절약해도 요금이 전혀 안 준다**는 점을 세 페이지 모두에 기재(헛된 절약 조언 방지).

### 구성
`tools/rubs-utility-bill-checker.html` / `blog/is-my-apartment-utility-bill-fair.html` / `compare/rubs-vs-submetering.html` / 용어집 RUBS·Submetering·Shared Meter

### 오디언스 관점
세입자 라인이 이제 4개 클러스터로 확장됨: 커뮤니티솔라 → 발코니솔라 → (v26) RUBS. **집을 소유하지 않은 사용자 전용 자산군이 형성됐으니, 다음에 세입자 주제를 다룰 때 이들끼리 상호링크할 것.**

---

# EcoEnergyCalc 인수인계 — v25 세션 (2026-09-07)

## 0-W. ✅ v25 신규 클러스터: 2028 화로 규제 (95% AFUE) — 커밋 `ac53a96`, 134 → 137파일

**GSC 밖 탐색으로 발굴.** 2026년 가전 효율기준 입법 뉴스에서 포착한, **기한이 확정된 소비자 결정** 주제.

### 배경 사실 (다음 세션에서 상태 변화 반드시 재확인할 것)
- **2028-12-18부터 주택용 가스화로는 95% AFUE 이상 의무** → 비콘덴싱 80% 생산 종료. DOE가 2025년 말 법적 다툼 끝에 기준 유지 확정.
- ⚠️ **유동적임**: `Energy Efficiency Reform Act of 2026`(Sen. Mike Lee) 발의됨(v25 시점 약 2주 전). 가스 순간온수기 규정은 2025-05-09 CRA로 이미 폐기(재발의 불가). DOE는 17개 기준 철회 제안 중(Consumer Reports 추산 소비자 부담 +$540억). **2028 날짜가 바뀌면 이 클러스터 3페이지 전부 갱신 필요.**
- DOE 공식 추산: 화로 기준으로 가구당 생애 순절감 ~$350, 연 최대 ~$50. 약 1/20 가구가 히트펌프로 전환 예상.

### 중복 확인
`non-condensing` 0건, 2028 규제 언급 0건. AFUE는 산발 언급만 있었고 업그레이드 경제성·배관 개조를 다루는 페이지 없었음. 기존 `heat-pump-vs-furnace-vs-boiler`는 95% AFUE를 전제로만 사용.

### 경쟁 회피 장치 (유지할 포지션)
경쟁은 HVAC 업체 블로그와 뉴스 기사뿐, **계산기 없음**. 차별화 3가지:
1. **"규제 전에 싼 80% 사두자"는 통념을 수치로 반증.** 80→95%는 15%p가 아니라 **비율 1−(0.80/0.95)=15.8%** 절감. 700therms/$1.70 기준 연 $188, 프리미엄 $1,200 회수 6년, 18년 순 **+$3,199**. 툴과 compare 두 계산기가 독립 경로로 동일하게 $3,199를 산출하는 것 교차검증함.
2. **"손익분기 프리미엄" 출력** — 배관 견적이 얼마를 넘으면 싼 유닛이 이기는지 역산. 온난기후+비싼 배관(350therms/$1.10/프리미엄 $2,800)에서는 실제로 비콘덴싱 승리로 정직하게 뒤집힘.
3. **DOE 수치가 왜 작아 보이는지 해명** — 전국 평균이고 구입가 차감 후이며 히트펌프 전환 가구가 섞임. 경쟁사가 안 하는 설명.

### 중립성·안전장치 (건드리지 말 것)
- 정책 논쟁 주제라 **양측 논거를 병기**함(효율 옹호측 생애비용 절감·CR 설문 87% 지지 vs 가스업계 장비비 상승·선택권 축소). 한쪽으로 기울이지 말 것.
- **"작동 중인 화로를 교체할 필요 없다"**(규제는 제조일 기준이지 소유 기준이 아님)를 툴·블로그·compare 세 곳 모두에 명시. 이 문장은 반드시 유지할 것 — 불필요한 교체를 유도하지 않기 위한 장치.

### 구성
`tools/furnace-afue-upgrade-calculator.html`(역전 입력 방어 포함) / `blog/furnace-efficiency-rule-2028.html` / `compare/condensing-vs-non-condensing-furnace.html` / 용어집 Condensing·Non-Condensing Furnace 신설 + 기존 AFUE 항목에 2028 규제 반영
...
**v22와 같은 방식(GSC 밖 탐색)으로 진행.** 2026년 전기요금 분노 관련 보도·레딧 인용에서 반복 등장하는 실제 문장을 포착: *"사용량은 20% 줄었는데 요금은 33% 늘었다"*, *"실제 전력비보다 delivery fee가 더 크다"*, *"$200 중 $130이 delivery"*.

### 배경 수치 (본문에 인용, 다음 세션에서 갱신 확인할 것)
- 2025년 요금인상 신청 **사상 최고 약 $310억** (전년의 2배), 2026 상반기만 **$186억** 추가
- 주택용 소매 전기요금 2025년 한 해 **+7%**, 미국 평균 2022년 ~15¢ → 2026년 **18.4¢**
- 2026-03 갤럽: 미국인 **10명 중 7명이 지역 데이터센터 건설 반대**
- ⚠️ **데이터센터 영향은 양방향 증거가 존재함**: EPRI 연구는 2024년까지 대형부하가 오히려 평균가격을 *낮췄다*고 봄(고정비 분산 효과). 본문에 이 점을 균형있게 서술했으니, 한쪽으로 단정하는 방향으로 수정하지 말 것.

### 중복 확인
`delivery charge` / `distribution` / `supply charge` / `capacity charge` 전 사이트 **0건**. 기존 `electric-bill.html`은 단일 혼합요율만 받고, `electric-bill-spike-calculator.html`은 청구액만 받아 요율 분해가 **구조적으로 불가** → 카니발 없는 보완재로 성립.

### 경쟁 회피 장치 (다음 세션이 유지해야 할 포지션)
경쟁은 (a)정적 해설 글 (b)**공급사 제휴 사이트**(choosemypower, electricrates.org, electricchoice, abcenergy 등)뿐. 둘 다 "청구서 1장 해설"에 그치고, 제휴 쪽은 절감액을 총청구액 대비로 부풀림.

1. **청구서 2장 비교 분해 — 시장에 없는 기능.** 사용량 / 공급요율 / 배송요율 / 고정료 / 세금 5개 요인으로 분해하며 **합산 항등식이 정확히 성립**(레딧 케이스: 사용량 −$32인데 요율 +$64로 상쇄, 총 +$41.04가 5요인 합과 소수점까지 일치). 지배 요인에 따라 처방을 분기 — 배송이 원인이면 공급사 변경 무의미, 고정료가 원인이면 절약 무의미, 사용량이 원인이면 그때만 효율 개선 권유.
2. **중립 포지션** — 공급사 변경이 청구서의 몇 %만 건드리는지 명시. 검증: 공급13.5¢/배송10.5¢/고정$15면 shoppable **53%**, 11¢ 오퍼는 공급요율 기준 19% 할인이지만 **총액으로는 9.8%**. 월 수수료 포함 시 더 싼 헤드라인 요율이 오히려 **손해**로 판정되는 케이스도 정확히 잡아냄.

### 구성
- `tools/bill-increase-breakdown-calculator.html`
- `blog/why-did-my-electric-bill-go-up-when-i-used-less.html` — 검침추정·청구일수 같은 **지루한 원인을 먼저 배제**하도록 구성(계량기 고장부터 의심하는 걸 막음)
- `compare/supply-vs-delivery-charges.html`
- 용어집 3항목: Delivery Charge, Supply Charge, Capacity Charge

### 알려진 한계 (본문에 명시함)
계산기는 양 기간 모두 **정액 요율 구조**를 가정한다. 누진제·TOU 요금제에서는 실효요율이 사용 패턴에 따라 움직여 "사용량 vs 요율" 분리가 근사치가 된다(총합은 여전히 정확). 세율은 양 기간 동일 적용, 청구일수 차이는 사용량 증가로 계상됨.

---

# EcoEnergyCalc 인수인계 — v22 세션 (2026-08-24)

## 0-Z. ✅ v22 신규 클러스터: Balcony Solar (플러그인 태양광) — 커밋 `01e441b`, 127 → 130파일

**이번 세션은 GSC/Bing 데이터 밖에서 니치를 찾는 방식으로 진행함.** 우리 데이터는 이미 있는 페이지의 성과만 보여주지 신규 기회를 알려주지 않기 때문. 레딧 고전기요금 스레드에서 "balcony plug-in solar panels" 언급이 반복 등장하는 걸 포착 → 웹 검증.

### 왜 지금 이 주제인가
- 미국은 2025년 3월 **유타 HB 340**이 최초 합법화. 표준 계통연계 규정이 5~25kW 지붕형 기준이라 400W 발코니 패널도 사실상 불법이었던 걸, 유타가 "소형 휴대 태양광 기기" 별도 카테고리로 예외 신설(독일 800W Balkonkraftwerk 프레임워크 모델).
- **2026년 8월 기준 8개 주 시행**: 유타·메인·버지니아·콜로라도·메릴랜드·코네티컷·버몬트·뉴햄프셔. **약 22개 주가 지난 1년간 발의** — 계속 늘어나므로 이 목록은 다음 세션에서 반드시 재확인할 것.
- 독일 100만+ 등록 시스템 vs 미국 문서 축적 15개월치 → **관심 급증 + 문서 희소**의 전형.

### 경쟁 회피 장치 (다음 세션이 유지해야 할 포지션)
경쟁은 (a)키트 판매 벤더(ecoflow, ussolarsupplier, balconysolar.uk — 대부분 제휴링크형) (b)입법 트래커뿐. **중립적 페이백 계산기가 없음.** 우리 차별화 2가지:

1. **제로수출 폐기분 모델링** — 플러그인은 수출이 불가해서 상시부하를 초과하는 생산분이 크레딧되지 않고 그냥 버려진다. 벤더 계산기는 전량 사용을 가정해 저축액을 부풀림. 우리는 일조 곡선(sin)을 12시간 창에 적분하고 시간대별 `min(생산, 상시부하)`로 실사용분만 계산.
   - 검증 결과: 상시부하 300W → 자가소비 88% / 페이백 10.2년 · 120W → 43% / 21년 · CA(500W, 6시간, 33¢) → 98% / 3.7년.
   - **핵심 메시지: 같은 장비·같은 가격인데 3.7~21년으로 벌어진다. 패널 크기가 아니라 상시부하가 결정한다.**
2. **30% 연방세액공제가 2025-12-31 만료**됐는데 경쟁 페이지 상당수가 아직 "공제 후" 가격/페이백을 싣고 있음. 우리는 전액 지불 기준으로 계산하고 이 점을 본문에 명시.

### 구성
- `tools/balcony-solar-payback-calculator.html` — 상시부하 찾는 법 3가지(유틸리티 인터벌 데이터 / 전력 모니터 / 가전 추정) 포함
- `blog/is-balcony-solar-worth-it.html` — 주별 합법 현황, **임대차·HOA가 주법을 덮지 못하는 문제**(유타만 세입자 명시 보호, 플로리다는 아파트 난간을 명시적으로 제외), UL 3700 인증 갭(2025-12 발행, 2026-01 시험 개시라 완제품 인증 미비 다수)
- `compare/balcony-solar-vs-community-solar.html` — 지붕 없는 사람의 두 선택지 직접 비교. **기존 커뮤니티솔라 클러스터와 상호 연결**해 세입자 오디언스를 하나로 묶음
- 용어집 2항목: Balcony Solar, Zero Export

### 다음 세션 확인 사항
- 주별 합법 현황은 빠르게 바뀐다. 캘리포니아 SB 868이 2026-08-31까지 하원 통과해야 진행되며 통과 시 2027-01-01 발효 — **CA가 열리면 이 클러스터 가치가 크게 오르므로 반드시 확인할 것.**
- UL 3700 인증 완제품이 나오기 시작했는지 확인. 나왔다면 본문의 "인증 갭" 서술을 갱신해야 함.

---

# EcoEnergyCalc 인수인계 — v21 세션 (2026-08-17)

## 0-A. ⚠️⚠️ 전략 재정의: Bing이 주력 채널로 확정됨 (v21 최대 발견)

**GSC만 보면 이 사이트는 실패한 것처럼 보이지만, 실제로는 Bing에서 이미 성공하고 있다. 앞으로 성과 판단은 Bing Webmaster + GA를 기준으로 할 것.**

| 지표 | Google (GSC, 3개월) | Bing (WMT) |
|---|---|---|
| 노출 | 445 | 340 (페이지 기준) |
| **클릭** | **0** | **6** |
| **평균 순위** | **60~90위** | **5.32위** |
...
- GA 세션(7/20~8/16): Bing계열 합계 **72세션**(bing 28 + duckduckgo 28 + yahoo 9 + ecosia 7) — 전주 36에서 **2배**. google/organic은 여전히 **0**. copilot 5.
- GA 전체: 활성 135명(전 106), 이벤트 870(전 750), 참여시간 51초(전 39초) — 전 지표 개선.
- **Bing 쿼리는 대부분 길고 대화체**("on adverage, how much water does it take to do a 5 star washing machine load twice in a day") — AI 어시스턴트 경유 트래픽으로 보임. 이게 우리가 롱테일·질문형 콘텐츠를 계속 만들어야 하는 이유.

### Bing 페이지 순위 (실제 돈이 되는 자산)
water-usage 187@6.19 · washing-machine-use(blog) 32@5.13 · dryer-energy-cost 25@5.80 · run-a-dryer(blog) 22@6.14 · washing-machine-water-usage 20@5.35 · heating-cost-by-fuel 14@4.50 · driving-vs-flying-carbon 11@4.64

**→ Bing 노출의 70%가 물 클러스터다.** 태양광 라인이 수익화 1순위라는 기존 방침은 유지하되, **트래픽이 실제로 발생하는 곳은 물/건조기 클러스터**라는 점을 인지하고 양쪽을 병행할 것.

## 0-B. ✅ v21 신규: Water Bill Benchmark 클러스터 (커밋 `33cea1e`, 125 → 127파일)

**갭 발견 경로**: Bing 롱테일 쿼리가 반복적으로 "내 청구서 숫자가 정상이냐"를 묻는데(`is 800 cf in one month for 3 person house normal or alot`, `how many cf's of water are used per month in a home with two people` ← **클릭 발생**), 기존 `water-usage.html`은 역방향(습관→추정)만 지원해 답을 못 주고 있었음.

**경쟁 조사 결과**: 시장에 (a)단순 CCF→갤런 변환기 (b)요금 계산기 (c)정적 EPA 페이지만 존재. **"청구서 숫자 → 가구원수 정규화 → 정상 여부 판정"을 하는 도구가 없음.** 경쟁이 비어 있는 각도.

- `tools/water-bill-benchmark-calculator.html` — 6개 단위(CF/CCF/gal/천갤런/m³/L) → gal/person/day 정규화 → 판정
- `blog/how-much-water-should-a-family-of-4-use.html` — 가구원수별 벤치마크

**차별화 4가지(다음 세션이 유지해야 할 포지션)**:
1. **CF vs CCF 혼동**(100배 차이)을 전면 경고하고 자릿수로 판별하는 법 제시 — 경쟁사가 안 다루는 실사용자 최대 함정
2. **청구주기 일수 입력** — 격월/분기 청구서를 월로 오독하는 흔한 오해 차단
3. **실외급수 여부로 벤치마크 조정**(82/105/145 gpcd) — 무조건 82와 비교하는 건 부정확
4. **"볼륨은 정상인데 청구서가 비싸다"면 요금구조 문제**라고 명시 — 누진/하수도/고정료
...
- **v20에서 홈 링크를 추가한 미크롤 7페이지는 여전히 `1970-01-01`(미크롤)** — 7일 경과했지만 변화 없음. 홈 링크 추가만으로는 부족했다는 뜻. 다음 세션에서도 그대로면 다른 수단(사이트맵 우선순위 상향, 해당 페이지 콘텐츠 보강)을 고려할 것. 단 **Bing에서는 이 중 일부가 이미 노출 중**(solar-shingles, tankless, electric-vs-gas-dryer, gas-dryer-vs-heat-pump)이므로 구글만의 문제임.

## 0-D. Google 쪽 관찰 (참고용, 행동 근거로 삼지 말 것)

- 3개월 누적 노출 445, 클릭 0. 8/09~8/17 구간 52노출 @74.5위.
- 냉장고 페이지가 유일한 성과: `how much does refrigerator cost` **20노출 @18.1위**, 페이지 35노출 @33.6(전주 27.9에서 개선).
- 태양광 CO2 클러스터(56노출 @~91위)는 v20의 홈 앵커 변경 후에도 순위 변화 없음. 구글 자체가 이 사이트를 낮게 평가하는 상태라 온페이지 조정으로는 한계.

---

## 0. v20 세션 핵심 요약 (2026-08-10, Opus 분석 세션 — 코드 수정 없음, 진단만)

### 0-0-8. ✅ 필수 누락 클러스터 3종 신규 구축 (커밋 `dfb04ad`) — 파일 118 → 122

"경쟁이 있어도 사이트 정체성상 반드시 있어야 하는 것"을 기준으로 선정. compare/ 신규는 여전히 만들지 않음(0-0-5 재평가 대기).

1. **Net Metering (최대 구멍)** — 태양광 페이지를 12개나 가진 사이트에 net metering 전용 페이지가 **0개**였음. `tools/net-metering-calculator.html` + `blog/net-metering-vs-net-billing.html`.
   - **차별화 각도**: 경쟁사 대부분이 "NEM 3.0이 75% 깎였다"만 반복하는데, 이 사이트는 **자가소비분은 정책과 무관하게 소매가 그대로 유효**하다는 점을 전면에 세움. 자가소비 비율이 정책 종류보다 결과를 더 크게 좌우한다는 게 핵심 메시지.
   - 배터리 가치를 `(소매가−수출가) × 이전kWh`로 정량화해 "배터리 사야 하나" 질문에 숫자로 답함.
   - 롱테일: net metering vs net billing, NEM 3.0 영향, grandfathering, 수출 크레딧 확인 방법.

2. **EV Charging Cost** — EV 비교 페이지(`car-vs-ev-carbon`, `ev-vs-gas-true-cost` 등)는 있었지만 **충전비용 전용 계산기가 없었음**. `tools/ev-charging-cost-calculator.html`.
   - **차별화 각도**: 경쟁 조사에서 "TOU 최적화 누락(critical)"이 명시적 갭으로 확인돼 이를 핵심 기능으로 채택. 공공 급속충전 비율을 분리 입력받아 **집충전 불가 세입자 시나리오**를 모델링(경쟁사 계산기 대부분이 집충전만 가정).
   - "Level 1이 Level 2보다 싸다"는 흔한 오해를 정면 교정(kWh당 동일, 다만 오프피크 창 안에 못 끝내면 Level 1이 더 비쌀 수 있다는 미묘한 반전까지 설명).

3. **Community Solar** — 전 사이트 언급 **0건**. 이 사이트가 한 번도 안 건드린 **세입자·아파트 거주자** 오디언스. `tools/community-solar-savings-calculator.html`.
   - **차별화 각도(유일 포지션)**: 사업자 계산기들이 전부 숨기는 **"두 장 청구서" 구조**를 전면 배치. 고정요금을 별도 입력받아 실질 절감률이 광고 할인율보다 낮다는 걸 정직하게 표시(예: 10% 할인 광고 → 실제 7.2%).
   - 지붕형이 가능한 주택 소유자에겐 지붕형이 유리하다고 명시해 중립성 유지(기존 태양광 자산과 모순 방지).

**품질 검증 방법(앞으로 계산기 만들면 반드시 이렇게 할 것)**: node로 DOM 스텁 만들어 **계산기를 실제 실행**하고 출력값을 python으로 수동 교차검산. 이 과정에서 community solar의 5년 누적 계산 버그를 발견함 — 구독료가 (a)크레딧 성장 (b)자체 에스컬레이터로 **이중 증가**해서, 유틸리티 인상률이 에스컬레이터보다 높은데도 절감액이 줄어드는 모순이 있었음(설명문과 정반대). 에스컬레이터를 1년차 기준 독립 적용으로 수정. **구조 스캔만으로는 절대 못 잡는 종류의 버그임.**

**등록 완료**: sitemap 122 URL, 툴 허브 41카드, 블로그 허브, 홈페이지(툴카드 3 + 블로그카드 1), 용어집 3항목(net-metering, net-billing, community-solar), 기존 태양광 3페이지(`solar-panel-roi`, `solar-panel-guide`, `battery-storage`)에서 역링크.
