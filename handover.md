...
- **⚠️ 중요 사실 기록: NREL이 2025-12-01부로 "National Laboratory of the Rockies(NLR)"로 개편, 도메인 nrel.gov → nlr.gov 이전 중.** `developer.nrel.gov`는 2026-05-29 완전 폐기 확인됨. about.html에 넣은 pvwatts.nrel.gov 링크가 사용자 스크린샷으로 "안 간다" 제보돼서 발견/수정함(pvwatts.nlr.gov로 교정). **사이트 전체에 nrel.gov/NREL 언급이 몇 군데 더 있음(blog/how-many-solar-panels-do-i-need.html 수정완료, tools/solar-panel-savings.html·tools/wind-turbine.html의 maps.nrel.gov는 텍스트 언급이라 미수정 상태) — 다음 세션에서 외부 링크(특히 정부/기관 도메인) 추가하거나 점검할 때 nlr.gov 이전 여부 항상 재확인할 것.**
