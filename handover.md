# EcoEnergyCalc 인수인계 문서 v14 (2026-07-17 기준)

v13 문서 + 이번 세션(v13→v14) 내용 통합. 새 세션 시작 시 이 문서만 보고 이어서 작업 가능하도록 작성함.
...
- blog: 25개 (v14에서 are-carbon-offsets-worth-it.html, is-a-heat-pump-worth-it-without-tax-credit.html 신규 추가)
- compare: 41개
...
5. **백로그: compare 카테고리 전체(41개 중 상당수)가 600~900단어대로 얇음.** v12에서 얇음+고아 교집합 4개만 처리함 — 색인률이 계속 정체되면 나머지 compare 페이지들도 순차적으로 900+ 단어로 확장 검토.
...
8. **버그 재발 패턴 정리**: article-body 조기종료 버그가 v10/v11/v12에서 계속 나왔음(v13/v14는 재스캔 결과 0건이지만 v14에서 `<h2>` 짝 안 맞는 유사 패턴이 2건 새로 나왔음 — solar-incentives-by-state.html, heat-pump-vs-furnace.html, 둘 다 "빈 `<h2>` 태그가 다음 헤더 앞에 낑겨있는" 동일 패턴). 새 세션 시작하면 항상 정규식 재스캔부터 할 것: `re.findall(r'</p>\s*\n\s*</div>\s*\n\s*\n\s{10,}<h[23]>', content)` 그리고 **h2 짝 검증은 속성 포함해서 정확히 셀 것** — `len(re.findall(r'<h2[ >]', c))` vs `c.count('</h2>')` (단순 `c.count('<h2>')`는 `<h2 style="...">`처럼 속성 있는 태그를 놓쳐서 오탐 발생함, v14에서 이 실수로 false positive 3개 나왔다가 재확인함). 정적 3열 이상 비교표에 `table-scroll` 랩퍼 누락되는 패턴도 발견됨(hot-tub-running-cost.html) — 새 비교표 만들 때마다 `<div class="table-scroll"><table>...</table></div>`로 감싸는 걸 습관화할 것. **JS 계산기에서 `parseFloat(x)||기본값` 패턴도 위험 — 사용자가 유효하게 0을 입력해도 기본값으로 대체되는 버그 가능(ev-vs-gas-car.html에서 발견) — `isNaN()` 체크로 대체할 것.**
