...
- **판단: 심사 탈락/미승인 상태가 확인되면 재심사를 서두르지 말 것.** 현 트래픽에서 AdSense 승인의 기대수익은 월 $0.1 수준이고, 승인되더라도 광고 밀도가 리드젠 CTA 전환을 갉아먹음. 재심사는 **월 5,000 사용자 이상 + 리드젠 실적이 안 나올 때**만 의미 있음.
- **사용자에게 확인 필요(v14부터 5세션 연속 미확인)**: AdSense 심사 결과가 통과/탈락/보류 중 무엇인지. 이번에도 답을 못 받으면, 다음 세션에서 스크립트 태그 제거(성능 개선) 여부를 사용자에게 물어볼 것.
